...
| Archivo | Rol |
|---|---|
| `main.js` | `CONFIG` global (única fuente de configuración por cliente), inyección de vistas (`doGet`, `include`), diccionarios compartidos (`obtenerDiccionarioDestinatarios`, `obtenerMapaBodegaGeneral_`, `obtenerInfoKamPorSolicitante_`), contador de pendientes (`contarArchivosPendientes`) |
| `index.html` | Dashboard (tarjetas de cliente) + utilidades globales (`verificarPendientes`, `abrirModulo`, `volverDashboard`, `toggleLoading`, `mostrarError`, `mostrarAlertaExito`). Incluye el wizard unificado vía `<?!= include('wizard_ui'); ?>`. `abrirModulo` consulta el registro `CLIENTES_UI` (si el cliente está registrado y no tiene `flujoCustom` → `wizardInit(cliente)`; si no → fallback a `window.initModulo<CLIENTE>`; si tampoco existe → "Módulo no disponible") |
...
### 7. CSV de carga a SAP sin fila de encabezado (sesión 2026-09-01, PR #1 mergeado)
Los 3 motores (MEDIPIEL, CMX, CEN) armaban el CSV con `filasCSV.unshift(<headers>)` antes de unirlo con `;`/salto de línea, así que el archivo que se sube al cargador de SAP siempre traía una primera línea de texto con los nombres de columna — el cargador no la espera. Se quitó el `unshift` en los 3 motores y se borró `medipiel_getEstructuraCSVHeaders_` (quedaba sin otro uso). Sin cambio en el contenido de las filas de datos ni en su orden. Nota pendiente de confirmar: si algún envío terminara con 0 ítems válidos, el CSV resultante sería un archivo de 0 bytes en vez de solo-encabezado; MEDIPIEL ya aborta con error antes de llegar ahí si `filasCSV` queda vacío, falta confirmar que CMX/CEN no puedan llegar a ese caso desde la UI.

### 8. KAM: fuente única en `Asignacion_KAM` (sesión 2026-09-01, PR #2 mergeado)
MEDIPIEL, CMX y CEN resolvían el KAM asignado a un cliente de 3 formas distintas (detalle completo en la deuda técnica 16, ahora resuelta): MEDIPIEL cruzaba `Asignacion_KAM` (SAP ID → nombre) y luego `Iniciales de agente` (nombre → Código/Correo) — funcionaba, pero repetido dos veces en el propio archivo; CMX hacía el mismo cruce contra `Iniciales de agente` comparando por la columna equivocada (`TEAM`) y nunca encontraba nada; CEN leía Correo/Iniciales directo de `Asignacion_KAM`.

Se confirmó contra la hoja real que `Asignacion_KAM` ya trae **Correo** e **Iniciales** por cada SAP ID (columnas G y H) y que ese dato coincide, fila por fila, con `Iniciales de agente` — son dos copias manuales del mismo dato. Se decidió que **`Asignacion_KAM` es la única fuente**: `Iniciales de agente` queda sin ningún uso en el código (ninguno de los 3 motores la lee más).

Se agregó `obtenerInfoKamPorSolicitante_(solicitanteSap)` en `main.js` (mismo lugar que `obtenerDiccionarioDestinatarios`/`obtenerMapaBodegaGeneral_`) — una sola lectura por SAP ID, sin cruce por nombre — y los 3 motores lo consumen en vez de su propia lectura. Devuelve `null` si el SAP ID no existe en la hoja o no tiene KAM asignado (fila `"SIN KAM"`, sin Correo/Iniciales), y cada motor conserva su propio fallback de siempre (MEDIPIEL: `"XX"`/vacío; CMX/CEN: `"SARA CARDONA"`/`"SC"`/`sara.cardona@isdin.com`).
...
- Toda constante de negocio (SAP: clase de pedido/org ventas/canal/sector, solicitante, nombre del cliente, lista de precios, cómo se detectan sus archivos) va en `CONFIG.CLIENTES[cliente]` — **no hardcodeada en el motor**. Incluye el KAM, su correo y la ciudad de destino: el KAM (nombre/iniciales/correo) se lee siempre vía `obtenerInfoKamPorSolicitante_(solicitanteSap)` (main.js, única fuente: `Asignacion_KAM` — `Iniciales de agente` ya no se usa, ver deuda 16) y la ciudad de destino vía `Destinatarios`; nunca como literal de fallback salvo el default explícito cuando el SAP ID no tiene KAM asignado.
